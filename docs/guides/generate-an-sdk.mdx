...
<Steps>
  ...
  <Step title="Generate services">
    Emit Effect schemas, HTTP traits, and explicitly typed yieldable operations grouped into resource services.
  </Step>
  <Step title="Add provider behavior">
    Implement credentials, a Protocol layer, error mapping, retry policy, and any pagination conventions in the provider repository.
  </Step>
  <Step title="Prove determinism">
    Regenerate and fail when committed services or `coverage.json` drift. Review method totals and unsupported operations, and test generator adaptations with focused request-shape fixtures.
  </Step>
