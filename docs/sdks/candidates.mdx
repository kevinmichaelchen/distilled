...
New provider work begins after the shared engine and generator pass the promotion checks below. Runtime and generated operation shape follow the [Protocol architecture decision](/guides/protocol-architecture). A large operation count alone is not evidence of a correct SDK.
:::
...
- Build the provider against a declared shared-package release; downstream generation must remain reproducible without a local filesystem override.
...
- Prefer checked-in named documents for high-value workflows over an unbounded generic GraphQL surface unless an ADR chooses otherwise.
