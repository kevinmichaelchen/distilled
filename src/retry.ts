 ...
 * `makeRetryService`) and threads it into `API.make({ retry: <SDK>.Retry })`
 ...
 * generated operations pass `retry: <SDK>.Retry` so the policy applies to
 * every opted-in API call below it.
 *
 * @example
 * ```ts
...
export const capped = (max: Duration.Duration) =>
  Schedule.modifyDelay(({ duration }) =>
    Effect.succeed(
      Duration.isGreaterThan(duration, max) ? max : duration,
    ),
  );
